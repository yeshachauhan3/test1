...
function setInitialAccount() {
    ...
    // Pass the user's UID if available
}
...
function updateAccount(type, subaccountType = null) {
    ...
    // Log the transaction
    logTransaction(currentUser?.uid, transactionType, inputValue); // Pass the user's UID if available

    document.getElementById('accountValue').innerText = accountValue.toFixed(2);

    // Update main account value in Firebase
    updateMainAccountValue(currentUser?.uid); // Pass the user's UID if available
}
...
function updateMainAccountValue(username) {
    ...
    if (currentUser && currentUserData) {
        // Update main account value in Firebase
        set(ref(database, `user/${username}/account`), {
            value: accountValue
        });
    } else {
...
function logTransaction(username, type, amount, subaccountType = null) {
    // Use the username to construct the path for transactions
    const transactionPath = `user/${username}/transactions`;
    ...
    updateMainAccountValue(username); // Use the username here
}